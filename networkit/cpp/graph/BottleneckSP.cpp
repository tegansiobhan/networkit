...
#include "BottleneckSP.h"

#include <algorithm>

namespace NetworKit {

BottleneckSP::BottleneckSP(const Graph& G, node source, bool storePaths, bool storeStack, node target) : SSSP(G, source, storePaths, storeStack, target) {

}

void BottleneckSP::run() {
	...
	auto relax([&](node u, node v, edgeweight w) {
		if (distances[v] > std::max(distances[u], w)) {
			distances[v] = std::max(distances[u], w);
			if (storePaths) {
			...
			TRACE("pq size: ", pq.size());
		} else if (storePaths && (distances[v] == std::max(distances[u], w))) {
